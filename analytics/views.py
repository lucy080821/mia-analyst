...
def ai_chat_api(request):
    ...
    try:
        ...
        - NUMERIC AGGREGATION RULE: Many columns in user-uploaded datasets containing currency or prices (like `giá_vnđ`, `revenue`, `pricecfrusdmt`) are stored as TEXT because they contain commas, spaces, or currency symbols. If you apply math aggregations (SUM, AVG, MIN, MAX) to them, you MUST cast them to NUMERIC and handle empty strings. Example: `AVG(CAST(NULLIF(TRIM(REPLACE(REPLACE(CAST("pricecfrusdmt" AS TEXT), ',', ''), '$', '')), '') AS NUMERIC))`. Otherwise, PostgreSQL will throw a type error like "invalid input syntax for type numeric".
