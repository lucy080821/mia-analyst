...
def ai_chat_api(request):
    ...
    try:
        ...
        - NUMERIC AGGREGATION RULE: Many columns in user-uploaded datasets containing currency or prices (like `giá_vnđ`, `revenue`) are stored as TEXT because they contain commas. If you apply math aggregations (SUM, AVG, MIN, MAX) to them, you MUST cast them to NUMERIC using standard SQL. Example: `AVG(CAST(REPLACE(REPLACE("giá_vnđ_kg_chưa_vat", ',', ''), '$', '') AS NUMERIC))`. Otherwise, PostgreSQL will throw a type error.
